...
SCHEDULER = True
EPOCHS = 80
NAME = f"BATCH_16-{EPOCHS}epochs-40grid-12A-Gamma0_375"
PHYSIC_LEN = 12
GAMMA = 0.375
N_POINTS = 32
...
def run():
    ...
    train_dataset = MoleculesDataset(
        train_paths,
        n_points=N_POINTS,
        physic_len=PHYSIC_LEN,
        use_numba=True,
        gamma=GAMMA,
    )
    print("Train Dataset generato")
    test_data = MoleculesDataset(
        test_paths,
        n_points=N_POINTS,
        physic_len=PHYSIC_LEN,
        use_numba=True,
    ...
    for epoch in pbar:
        ...
        if test_mae < best_mae:
            torch.save(
                model.state_dict(), f"checkpoints/{NAME}-{date_time}/best_model.ckpt"
            )
            best_mae = test_mae
        if r2_test > best_r2:
            torch.save(
                model.state_dict(), f"checkpoints/{NAME}-{date_time}/best_model_r2.ckpt"
            )
            best_r2 = r2_test
